...
// MIPI
`define MIPI_DATA_NUM       24 // 1(freq)+3(delay)+1(sa)+1(cmd)+2(addr)+16(data)

`define MIPI_CLKDIV_NBIT    9 // 187.5KHz ~ 24MHz
...
`define MIPI_SA_BASEADDR    `MIPI_BUF_ADDR_NBIT'd4
...
`define MIPI_CDATA_BASEADDR `MIPI_BUF_ADDR_NBIT'd8
